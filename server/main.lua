            ...
            if Player.PlayerData.items[slot].info.quality == info.quality then
                ...
                TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'AddItem', 'green',
                    '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' ..
                        source .. ')** got item: [slot:' .. slot .. '], itemname: ' ..
                        Player.PlayerData.items[slot].name .. ', added amount: ' .. amount .. ', new total amount: ' ..
                ...
                for i = 1, Config.MaxInventorySlots, 1 do
                    if Player.PlayerData.items[i] == nil then
                        ...
                        TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'AddItem', 'green',
                            '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' ..
                                source .. ')** got item: [slot:' .. i .. '], itemname: ' ..
                                Player.PlayerData.items[i].name .. ', added amount: ' .. amount ..
                                ', new total amount: ' .. Player.PlayerData.items[i].amount)
                        return true
                    end
                end
            end
    ...
    if not ply.state.inv_busy then
        if name and id then
            ...
            elseif name == "trunk" then
                ...
                if (Trunks[id] and Trunks[id].isOpen) or
                    (QBCore.Shared.SplitStr(id, "PLZI")[2] and Player.PlayerData.job.name ~= "police") then
                    secondInv.name = "none-inv"
...
RegisterNetEvent('inventory:server:SetAsLastUseStashItem', function(item)
    ...
    -- print(json.encode(lastUsedStashItem.name, {ident = true}))
end)
                                    ...
                                    if toItemData.name ~= fromItemData.name then
                                        ...
                                        TriggerEvent("qb-log:server:CreateLog", "drop", "Swapped Item", "orange",
                                            "**" .. GetPlayerName(src) .. "** (citizenid: *" ..
                                                ...
                                                "*) swapped item; name: **" .. itemInfo["name"] .. "**, amount: **" ..
                                                toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" ..
                                                fromAmount .. "** - dropid: *" .. toInventory .. "*")
                                    end
                                else
                                    TriggerEvent("qb-log:server:CreateLog", "anticheat", "Dupe log", "red",
                                        "**" .. GetPlayerName(src) .. "** (citizenid: *" .. Player.PlayerData.citizenid ..
        ...
        elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
            ...
            elseif QBCore.Shared.SplitStr(shopType, "_")[1] == "market" then
                if Player.Functions.RemoveMoney("blackmoney", price, "blackmarket-item-bought") then
                    AddItem(src, itemData.name, fromAmount, toSlot, itemData.info)
                    TriggerEvent('mh-cashasitem:server:updateCash', src, itemData.name, toAmount,
                                                "remove", true)
                    QBCore.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
                                ...
                                if toItemData ~= nil then
                                    ...
                                    if toItemData.amount >= toAmount then
                                        if toItemData.name ~= fromItemData.name then
                                            ...
                                            TriggerEvent('mh-cashasitem:server:updateCash', src, toItemData, toAmount,
                                                "remove", true)
                                            AddToDrop(fromInventory, toSlot, itemInfo["name"], toAmount,
                                                toItemData.info, itemInfo["created"])
                                            if itemInfo["name"] == "radio" then
                                                TriggerClientEvent('Radio.Set', src, false)
                                            end
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot)
    ...
    if Player == OtherPlayer then
        return QBCore.Functions.Notify(src, "You can\'t give yourself an item?")
    end
    if dist > 2 then
        return QBCore.Functions.Notify(src, "You are too far away to give items!")
    end
    local item = GetItemBySlot(src, slot)
    if not item then
        QBCore.Functions.Notify(src, "Item you tried giving not found!");
        return
    end
    if item.name ~= name then
        QBCore.Functions.Notify(src, "Incorrect item found try again!");
        return
    end
...
QBCore.Commands.Add("giveitem", "Give An Item (Admin Only)", {{
    ...
    if Player then
        if itemData then -- check iteminfo
            local info = {}
            ...
            if AddItem(id, itemData["name"], amount, false, info) then
                QBCore.Functions.Notify(source, "You Have Given " .. GetPlayerName(id) .. " " .. amount .. " " ..
...
function ConvertQuality(item)
    ...
    local DecayRate = QBCore.Shared.Items[item.name:lower()]["decay"] ~= nil and
                          QBCore.Shared.Items[item.name:lower()]["decay"] or 0.0
...
QBCore.Functions.CreateCallback('inventory:server:ConvertQuality', function(source, cb, inventory, other)
    ...
    for _, item in pairs(inventory) do
        if item.created then
            if QBCore.Shared.Items[item.name:lower()]["decay"] ~= nil or QBCore.Shared.Items[item.name:lower()]["decay"] ~=
                0 then
                if item.info then
    ...
    if other then
        ...
        if inventoryType == "trunk" then
            for _, item in pairs(other.inventory) do
                if item.created then
                    if QBCore.Shared.Items[item.name:lower()]["decay"] ~= nil or
                        QBCore.Shared.Items[item.name:lower()]["decay"] ~= 0 then
                        if item.info then
                            if item.info.quality == nil then
                                item.info.quality = 100
                            end
